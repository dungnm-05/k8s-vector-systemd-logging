...
### Vector: Permission denied đọc log

```bash
# Triệu chứng
# ERROR Failed reading file for fingerprinting. error=Permission denied

# Fix: đổi User trong systemd service
nano /lib/systemd/system/vector.service
# User=root
# Group=root

systemctl daemon-reload && systemctl restart vector
```

### Vector: journald invalid record

```bash
# Triệu chứng
# WARN Invalid record from journald, discarding.

# Đây là bình thường — một số dòng journal bị truncate
# Vector bỏ qua và tiếp tục, không ảnh hưởng hoạt động
```

### Elasticsearch: index yellow

```bash
# Bình thường với 1-node cluster
# replica=0 trong index template đã được set → chỉ xảy ra với index cũ

# Fix index cũ nếu muốn green
curl -sk --cacert /etc/elasticsearch/certs/http_ca.crt \
  -u elastic:111111 \
  -X PUT "https://localhost:9200/k8s-logs-*/_settings" \
  -H "Content-Type: application/json" \
  -d '{"number_of_replicas": 0}'
```

### K8s Events không vào ES

```bash
# Kiểm tra kubectl chạy được không
kubectl get events --all-namespaces -o json | wc -c
# Phải > 0

# Vấn đề thường gặp: kubectl output nhiều dòng
# Fix đã áp dụng: dùng tr -d '\n' để gom thành 1 dòng
# command: bash -c "kubectl get events ... | tr -d '\n'"
```

### Kibana không thấy Data View

```bash
# Kiểm tra index đã có chưa
curl -sk --cacert /etc/elasticsearch/certs/http_ca.crt \
  -u elastic:111111 \
  "https://localhost:9200/_cat/indices/k8s-*"

# Nếu chưa có index → Vector chưa gửi data
# Kiểm tra: journalctl -u vector -f
```
